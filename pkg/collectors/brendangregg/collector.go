// Package brendangregg implement collector for https://www.brendangregg.com/blog/index.html
package brendangregg
...
func (c *Collector) Start(ctx context.Context, ch chan<- collectors.Post) error {
	...
	c.listCollector.OnHTML("#home", func(h *colly.HTMLElement) {
		h.ForEachWithBreak("li", func(_ int, h *colly.HTMLElement) bool {
			...
			err := c.postCollector.Request("GET", "https://www.brendangregg.com/"+path, nil, nil, nil)
			if err != nil {
	...
	c.postCollector.OnHTML("div.site", func(h *colly.HTMLElement) {
		title := h.ChildText("h2.big")
		if title == "" {
			return
		}

		publishedAt := h.ChildText("p.meta")
		t, err := time.Parse("02 Jan 2006", publishedAt)
		if err != nil {
			return
		}

		postBody, err := h.DOM.Find("div.post").Html()
		if err != nil {
			return
		}
	...
	err := c.listCollector.Request("GET", "https://www.brendangregg.com/blog/index.html", nil, colly.NewContext(), nil)
	if err != nil {
		return err
	}
