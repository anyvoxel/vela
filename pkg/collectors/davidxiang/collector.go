// Package davidxiang implement collector for https://davidxiang.com/
package davidxiang
...
func (c *Collector) Start(ctx context.Context, ch chan<- collectors.Post) error {
	...
	c.listCollector.OnHTML("#main", func(h *colly.HTMLElement) {
		h.ForEachWithBreak("article.post", func(_ int, h *colly.HTMLElement) bool {
			...
			err := c.postCollector.Request("GET", "https://davidxiang.com/"+path, nil, nil, nil)
			if err != nil {
	...
	c.postCollector.OnHTML("article.post", func(h *colly.HTMLElement) {
		title := h.ChildText("h1.post-title")
		if title == "" {
			return
		}

		publishedAt := h.ChildText("div.header-standard span time.entry-date")
		t, err := time.Parse(time.DateOnly, publishedAt)
		if err != nil {
			return
		}

		postBody, err := h.DOM.Find("div.post-entry div.entry-content").Html()
		if err != nil {
			return
		}
	...
	err := c.listCollector.Request("GET", "https://davidxiang.com/", nil, colly.NewContext(), nil)
	if err != nil {
		return err
	}
