// Package emptysqua implement collector for https://emptysqua.re/blog/all-posts/
package emptysqua
...
func (c *Collector) Start(ctx context.Context, ch chan<- collectors.Post) error {
	...
	c.listCollector.OnHTML("div.container", func(h *colly.HTMLElement) {
		h.ForEachWithBreak("ul.post-list li", func(_ int, h *colly.HTMLElement) bool {
			path := h.ChildAttr("a.summary-title", "href")
			if path == "" {
				return false
			}
	...
	c.postCollector.OnHTML("div.container article.post", func(h *colly.HTMLElement) {
		title := h.ChildText("h1.title a")
		if title == "" {
			return
		}

		publishedAt := h.ChildText("header time.post-time")
	...
	err := c.listCollector.Request("GET", "https://emptysqua.re/blog/all-posts/", nil, colly.NewContext(), nil)
	if err != nil {
		return err
	}
