// Package googlepubs implement collector for https://research.google/pubs/
package googlepubs
...
func (c *Collector) Start(ctx context.Context, ch chan<- collectors.Post) error {
	...
	c.postCollector.OnHTML("#page-content", func(h *colly.HTMLElement) {
		title := h.ChildText("h1.headline-2")
		if title == "" {
			return
		}

		postBody, err := h.DOM.Find("section.offset-two-up").Html()
		if err != nil {
			return
		}
	...
	err := c.listCollector.Request("GET", "https://research.google/pubs/", nil, colly.NewContext(), nil)
	if err != nil {
		return err
	}
