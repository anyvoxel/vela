...
func (c *Collector) Start(ctx context.Context, ch chan<- collectors.Post) error {
	...
	c.listCollector.OnHTML("body div.section div.content div.content ul", func(h *colly.HTMLElement) {
		h.ForEachWithBreak("li", func(_ int, h *colly.HTMLElement) bool {
			...
			err := c.postCollector.Request("GET", "https://arpitbhayani.me"+path, nil, nil, nil)
			if err != nil {
	...
	c.postCollector.OnHTML("body div.section div.columns", func(h *colly.HTMLElement) {
		title := h.ChildText("div h1.title")
		if title == "" {
			return
		}
